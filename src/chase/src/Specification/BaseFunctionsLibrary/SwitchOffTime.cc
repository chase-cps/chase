 ...
 *
 * @file	chase/src/Specification/BaseFunctionsLibrary/SwitchOffTime.cc
 * @brief
 */
...
SwitchOffTime::SwitchOffTime() :
    RequirementFunction("switch-off-time",3)
{
}
...
void SwitchOffTime::resolve( Requirement * req, Problem * problem )
{
            ...
            ++it )
    {
        ...
        problem->physical_switch_off.insert(*it);

        std::string component_name((*it)->getName());
        std::string counter_name = component_name + "_switch_off_counter";
        ...
        counter->setActivationCondition(
                new UnaryLogicCombination( op_lnot,
...
std::list< Behaviors::Timing::chase_time > * 
    SwitchOffTime::getTiming( Requirement * req )
{
